...
nyears = 1.0
Totaltime = nyears*365*24*60*60.0                 # seconds
TotalHours = round(Int, Totaltime/3600)         # hours
dt_FR = 2.0                                       # time step for FR
...
TIME_FR = collect(0.0:dt_FR:Totaltime)
...
function  getinitial(csp_avg, csn_avg, delta_sei, value, mode)
    m = Model(optimizer_with_attributes(Ipopt.Optimizer, "print_level" => 0, "tol" => 1e-10, "acceptable_tol" => 1e-8, "mu_strategy" => "adaptive"))
...
function simulation(TIME_FR_segment, signal_segment, FR_band, grid_band, u0)
	   TIME_FR_segment = collect(0.0:dt_FR:3600.0)
	   ...
	   tspan = (float(TIME_FR_segment[1]), float(TIME_FR_segment[end]))
        bspline_itp = extrapolate(interpolate(P_FR_segment, BSpline(Linear())), Flat())  
        function custom_interp(t)
            idx = t / dt_FR + 1.0 
            return bspline_itp(idx)
        end
           ...
           function f_FR(out,du,u,param,t)
            ...
            sol = DifferentialEquations.solve(prob, IDA(linear_solver=:Dense); 
...
function MPC(u0,batch,FR_price_sample, grid_price_sample)
    println("MPC start for batch $batch")
    start_time = time()
    i_start = 1
    i_end = 1
    ...
    for i_start in 1:Nt_FR_hour:(Nt_FR-Nt_FR_hour)
        ...
        if capacity_remain <= capacity_retire || i_start >= Nt_FR-Nt_FR_hour
	        ...
	        filename = string("SimuBatch", batch, ".jld")
            save(filename, "retire_time",retire_time,  "FR_band_list",FR_band_list, "buy_from_grid",buy_from_grid,
            "capacity_remain_list", capacity_remain_list, "soc_list", soc_list, "status_list", status_list, "stage_cost_list", stage_cost_list)
            break
        end

        signal_start = rand(signal_index)
        signal_end = signal_start+Nt_FR_hour
        signal_segment = signal[signal_start:signal_end]

    	FR_price_segment = FR_price_sample[i_start_hour:i_start_hour]
    	grid_price_segment = grid_price_sample[i_start_hour:i_start_hour]


        FR_band, grid_band, status, stage_cost = OptimalControl(u0,  signal_segment,  FR_price_segment, grid_price_segment, soc_min, soc_max)


        FR_band = max(min(FR_band,umax[1]),umin[1])
        grid_band = max(min(grid_band,umax[2]),umin[2])

        stage_cost_list[i_start_hour] = stage_cost

    	status_list[i_start_hour] = 1 
    	if status != MOI.OPTIMAL && status != MOI.LOCALLY_SOLVED
    	    status_list[i_start_hour] = 0
    	end

        if TIME_FR[i_start]%(3600*24) == 0
        ...
        train_x = [Statistics.mean(signal_segment), Statistics.var(signal_segment), FR_price_segment[1], grid_price_segment[1],soc,capacity_remain]
...
# price is $/KW, FR_band * Ncell is W, hence /1000
    Ncell = 1e6/P_nominal
    revenue = 0
    min_hours = min(TotalHours, length(FR_price_sample))
    for i in 1:min_hours
        price = FR_price_sample[i]
    	FR_band = FR_band_list[i]
    	revenue += price * FR_band *Ncell/1000
    end

    cost = 0
    min_hours = min(TotalHours, length(grid_price_sample))
    for i in 1:min_hours
        price = grid_price_sample[i]
