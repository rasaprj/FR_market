...
function OptimalControl(u0, signal_segment, FR_price_segment, grid_price_segment, soc_min, soc_max)
    ...
    Nt_FR_Horizon = round(Int, Total_time_horizon/dt_FR)+1           		
    ...
    @variable(m, -P_nominal*maxC <= power[1:Nt_FR_Horizon] <= P_nominal*maxC) 
...
function control_start(batch_num)
    for batch = 1: batch_num
        println("Starting batch $batch / $batch_num...")
       ...
       println("Completed batch $batch / $batch_num.")
    end
