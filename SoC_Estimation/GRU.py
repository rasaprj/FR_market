...
class Config:
    ...
    hidden_size: int = 64
    num_layers: int = 2
    dropout: float = 0.1  # only applied if num_layers > 1
...
class SeqDataset(Dataset):
    """
    For target at time t, input is X[t-seq_len+1 : t+1], target is y[t].
    ...
    def __getitem__(self, i: int):
        ...
        x_seq = self.X[t - self.seq_len + 1 : t + 1]  # (T, F)
        y_t = self.y[t]                               # (1,)
...
class GRURegressor(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        # x_seq: (B, T, F)
        out, _ = self.gru(x_seq)      # out: (B, T, H)
        last = out[:, -1, :]          # (B, H)
...
def main():
    ...
    if best_y is not None and best_yhat is not None:
        ...
        plt.ylabel("SoC")
        plt.title("Best GRU: Predicted vs Actual (Validation Block)")
        plt.legend()
        ...
        subset_size = max(1, int(0.25 * n_points))
        ...
        plt.title("Best GRU: Actual vs Predicted (25% Sample)")
        plt.tight_layout()
