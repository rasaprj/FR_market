...
function run!(actor, aopt, critic, copt, env;
        epochs=50, steps=168*epoch_week, maxt=168, batchsize=160, noisescale=5f-2, γ=0.9, τ=0.01, anchor_weight=0.5f0) ## steps  = 168*4
    ...
    for i in 1:epochs
        ...
        done, total_r1, total_r2 = episodes!(memory, env, steps, maxt, FR_price_sample, grid_price_sample, signal_segment) do s 
...
BSON.@load "RL_Pretrain/actor_model_rl_DA.bson" actor
BSON.@load "RL_Pretrain/critic_model_rl_DA.bson" critic
...
function simulate_and_plot(actor, env, FR_price, grid_price, signal, maxt)
    reset!(env)
    soc_list = Float64[]
    ...
    reward1_list = Float64[]  # Operational profit per hour
    reward2_list = Float64[]  # Total reward per hour
    ...
    for i in 1:maxt
        ...
        set!(env, FR_price[i], grid_price[i], signal[(i-1)*Nt_FR_hour+1:i*Nt_FR_hour+1])
        s = env.current
        action = Float64.(actor(s))
        fr_band = denormalizeU(vec(action), 1)
        grid_band = denormalizeU(vec(action), 2)
        push!(fr_band_list, fr_band)
        push!(grid_band_list, grid_band)
        current, r1, r2, done = step!(env, action, FR_price[i], grid_price[i], signal[(i-1)*Nt_FR_hour+1:i*Nt_FR_hour+1], i)
        soc_end = current[5]
        capacity_remain = current[6]
        u0 = env.state
