...
function simulation(TIME_FR_segment, signal_segment, FR_band, grid_band, u0)
       ...
       itp = linear_interpolation(TIME_FR_segment, P_FR_segment)
           ...
           function f_FR(out,du,u,param,t)
            ...
            sol = DifferentialEquations.solve(prob, IDA(), verbose=false)
...
function step_battery(FR_price_segment, grid_price_segment, signal_segment, FR_band, grid_band, u0, hour)
    ...
    #tracking_penalty = -0.0005 * (FR_band - fr_mpc)^2 - 0.0005 * (grid_band - grid_mpc)^2
    tracking_penalty = - 5 * ( ((FR_band - fr_mpc) / P_nominal)^2 + ((grid_band - grid_mpc) / P_nominal)^2 )
