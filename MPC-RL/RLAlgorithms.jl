module RLAlgorithms
    ...
    function episode!(f, memory, env, maxt, FR_price, grid_price, signal, start_hour)
        ...
        while t < maxt 
            ...
            set!(env, FR_price_segment, grid_price_segment, signal_segment, start_hour + t)
            s = copy(env.current)
            a = f(s)
    ...
    function test(f, env, FR_price_segment, grid_price_segment, signal_segment, maxt=28*24) where {S, R, A}
        ...
        for i in 1:maxt
            set!(env, FR_price_segment[i], grid_price_segment[i], signal_segment[(i-1)*Nt_FR_hour+1:(i)*Nt_FR_hour+1], i)
            s = env.current
