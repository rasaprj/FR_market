...
function OptimalControl(u0, t_start, soc_min, soc_max)
    ...
    set_optimizer_attribute(m, "max_cpu_time", 6000.0) 	 #default linear solver mumps
    ...
    if soc_min != soc_max
        ...
        JuMP.set_lower_bound(csn_avg[Nt], capacity_remain*soc_min*csnmax)
    	JuMP.set_upper_bound(csn_avg[Nt], capacity_remain*soc_max*csnmax)
    else
    ...
    println("Solver status: ", status)  # Add this for debugging

    if status == MOI.LOCALLY_SOLVED || status == MOI.OPTIMAL
        FR_band_val = value.(FR_band)[1]
        grid_band_val = value.(buy_from_grid)[1]
        cf_fade = (value.(cf[Nt]) - cf0) / Qmax  # Compute fade for print

        println("FR_price:  ", FR_price[nHours_start+1])
        println("FR_band:  ", FR_band_val)
        println("buy_from_grid:  ", grid_band_val)
        println("buy_from_grid_plus:  ", value.(buy_from_grid_plus[1]))  # This is safe since optimal

        println("revenue   ", FR_price[nHours_start+1] * FR_band_val)
        println("cost      ", value.(buy_from_grid_plus[1]) * grid_price[nHours_start+1])
        println("penalty   ", expectedrevenue / (1 - capacity_retire) * cf_fade)
        println("capacity fade:  ", cf_fade)
    else
        # Fallback heuristic
        FR_band_val = 3 * P_nominal
        power_next = P_nominal * soc0 + FR_band_val * mean(signal[Nt_FR_start+1:Nt_FR_start+Nt_FR_Horizon])
        if power_next <= P_nominal * capacity_remain * soc_min
            grid_band_val = (P_nominal * capacity_remain * soc_min - power_next) / nHours_Horizon
        elseif power_next >= P_nominal * capacity_remain * soc_max
            grid_band_val = (P_nominal * capacity_remain * soc_max - power_next) / nHours_Horizon
        else
            grid_band_val = 0
        end

        # Optional: Print fallback info (no cf_fade available without solve)
        println("FR_price:  ", FR_price[nHours_start+1])
        println("FR_band (fallback):  ", FR_band_val)
        println("buy_from_grid (fallback):  ", grid_band_val)
        println("No revenue/cost/penalty/fade computed due to non-optimal solve")
    end

    return FR_band_val, grid_band_val
end
